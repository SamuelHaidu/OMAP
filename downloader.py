...
DOWNLOAD_PATH = __main__.DOWNLOAD_PATH
class Downloader(Thread):
    ...
    def run(self):
        if self.callback and self.tp == 'ogg':
            ...
            self.video.getbestaudio(preftype='webm').download(filepath=DOWNLOAD_PATH+self.video.title+'.ogg',
                                               quiet=False)
    ...
    def mp3_downloader(self, video_id):
        ...
        self.f = open(DOWNLOAD_PATH+self.video.title+'.mp3', 'wb')
        ...
        while True:
            ...
            self.eta = (self.total-self.recvd)/self.rate
